...
MAX_BLUR_SEARCH = 5
SEARCH_WINDOW = 5
MIN_DIFF = 0.2  # important fix - difference of sharp vs blur in non-OIS to ensure quality
...
def score_frames(folder, capture_name, cam_type):
    ...
    # reuse logs if already computed
...
def select_blur(values, segment, threshold):
    best_idx = segment[0]
    best_score = values[best_idx]

    for idx in segment[:MAX_BLUR_SEARCH]:
        score = values[idx]

        # strong blur → accept immediately
        if score <= threshold:
            return idx
...
def process_capture(capture_name, scene_id):
    ...
    for i in range(len(ois_segments) - 1):
        ...
        blur_idx = select_blur(ois_values, blur_seg, threshold_ois)

        SEARCH_WINDOW = 5

        # --- Find non-OIS sharp near OIS sharp ---
        sharp_idx_nonois = min(
            range(len(nonois_values)),
            key=lambda x: abs(x - sharp_idx)
        )

        # refine: search best sharp nearby
        s_start = max(0, sharp_idx_nonois - SEARCH_WINDOW)
        s_end = min(len(nonois_values), sharp_idx_nonois + SEARCH_WINDOW)

        sharp_idx_nonois = max(
            range(s_start, s_end),
            key=lambda x: nonois_values[x]
        )

        # --- Find non-OIS blur near OIS blur ---
        blur_idx_nonois = min(
            range(len(nonois_values)),
            key=lambda x: abs(x - blur_idx)
        )

        # refine: search best blur nearby
        ...
        diff = nonois_values[sharp_idx_nonois] - nonois_values[blur_idx_nonois]
        ...
        if DEBUG_MODE:
            ...
            nonois_sharp_img = read_image(os.path.join(nonois_dir, nonois_results[sharp_idx_nonois][0]))
            nonois_blur_img = read_image(os.path.join(nonois_dir, nonois_results[blur_idx_nonois][0]))
