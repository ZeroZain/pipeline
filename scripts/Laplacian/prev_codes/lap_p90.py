...
SHARP_WINDOW = 10
BLUR_WINDOW = 4
BLUR_TO_SHARP_MAX_RATIO = 0.8 # Blur threshold = this ratio of the pre-drop sharp baseline.
...
def find_transitions(values):
    ...
    for i in range(SHARP_WINDOW, len(values) - BLUR_WINDOW):
        ...
        blur_limit = sharp_ref * BLUR_TO_SHARP_MAX_RATIO
        immediate_blur = values[i:i + BLUR_WINDOW]
        if not all(v <= blur_limit for v in immediate_blur):
            continue

        future_end = min(len(values), i + STABLE_CHECK)
        future = values[i:future_end]
        if all(v <= blur_limit * 1.05 for v in future):
...
def process_capture(capture_name, scene_id):
    ...
    for t in transitions:
        ...
        if nonois_values[blur_idx_nonois] > (sharp_ref_nonois * BLUR_TO_SHARP_MAX_RATIO):
            continue
