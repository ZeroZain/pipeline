...
BLUR_PERCENTILE = 25 # When determining the sharpness threshold for a video, use this percentile of the OIS sharpness scores. Adjust based on your data. 30 means using the 30th percentile, 20 means using the 20th percentile (more aggressive), 40 means using the 40th percentile (more conservative), etc.

# controls transition detection and frame pick ranges
SHARP_WINDOW = 10
BLUR_WINDOW = 4
BLUR_TO_SHARP_MAX_RATIO = 0.9 # OIS blur threshold ratio tuned to recover missed transitions in benchmark logs.
...
STABLE_MAX_OUTLIERS = 4 # Allow this many brief spike frames in the stable-check window.
STABLE_LOW_PERCENTILE = 80 # Require this percentile of the stable window to remain below blur limit.
OIS_IMMEDIATE_MAX_OUTLIERS = 2 # OIS immediate-drop gate: allow this many spikes in BLUR_WINDOW.
NONOIS_IMMEDIATE_MAX_OUTLIERS = 2 # non-OIS immediate-drop gate: allow this many spikes in BLUR_WINDOW.
...
DEBUG_MODE = True
DEBUG_OUTPUT = os.path.join(DEBUG_ROOT, "debug_vis")
...
def load_processed_captures_from_log():
    processed = set()

    if not os.path.exists(SCENE_LOG):
        return processed

    with open(SCENE_LOG, "r", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            capture = row.get("capture", "").strip()
            if capture:
                processed.add(capture)

    return processed


def next_scene_id_from_log():
    max_scene_id = 0
    ...
    with open(SCENE_LOG, "r", newline="") as f:
        ...
        for row in reader:
            ...
            match = SCENE_PATTERN.search(scene_name)
            if match:
...
def append_scene_log(
    ...
    ois_drop_frame,
    nonois_drop_frame,
    ois_drop_frame_actual,
    nonois_drop_frame_actual,
    ...
    append_csv(
        ...
        [
            ...
            ois_drop_frame_actual,
            nonois_drop_frame_actual,
            "true" if nonois_used_fallback else "false",
            "true" if is_linear_scene else "false",
        ],
    )
...
def process_capture(capture_name, scene_id):
    ...
    for t in transitions:
        ...
        # Neighboring transition candidates can map to the same selected frames.
        # Keep each selected scene unique within a capture.
        frame_key = (
            sharp_file,
            blur_file,
            nonois_sharp_file,
    ...
    if scene_count < MAX_SCENES_PER_VIDEO:
        tqdm.write(
            f"{capture_name}: requested {MAX_SCENES_PER_VIDEO} scenes, built {scene_count} unique scenes."
        )
...
def main():
    ...
    processed_captures = load_processed_captures_from_log()
    scene_id = next_scene_id_from_log()

    for capture in tqdm(captures, desc="Processing"):
        capture_posix = to_posix(capture)
        if capture_posix in processed_captures:
            tqdm.write(f"Skipping (already in scene log): {capture}")
            continue
